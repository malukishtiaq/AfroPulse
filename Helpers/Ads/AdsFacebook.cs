...
namespace PlayTube.Helpers.Ads
{
	public static class AdsFacebook
	{
		...
		public static AdView InitAdView(Activity activity, LinearLayout adContainer, RecyclerView recyclerView)
		{
			try
			{
				if (AppTools.GetStatusAds() && AppSettings.ShowFbBannerAds)
				{
		...
		public static InterstitialAd InitInterstitial(Activity activity)
		{
			try
			{
				if (AppTools.GetStatusAds() && AppSettings.ShowFbInterstitialAds)
				{
		...
		public static RewardedVideoAd InitRewardVideo(Activity activity)
		{
			try
			{
				if (AppTools.GetStatusAds() && AppSettings.ShowFbRewardVideoAds && CountRewarded == AppSettings.ShowAdRewardedVideoCount)
				{
					CountRewarded = 0;
		...
		public static void InitNative(Activity activity, LinearLayout nativeAdLayout, NativeAd ad)
		{
			try
			{
				if (AppTools.GetStatusAds() && AppSettings.ShowFbNativeAds)
				{
					if (ad == null)
					{
		...
		public static void BindAdFb(Context context)
		{
			try
			{
				switch (AppTools.GetStatusAds() && AppSettings.ShowFbNativeAds)
				{
