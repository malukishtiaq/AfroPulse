...
using PlayTube.PlayTubeClient.Classes.Live;
	...
	public class LiveStreamingActivity : RtcBaseActivity, IEventHandler, IDialogListCallBack/*, PixelCopy.IOnPixelCopyFinishedListener*/
	{
		...
		protected override void OnCreate(Bundle savedInstanceState)
		{
			try
			{
				...
				{
					case false:
						LiveStreamViewerObject = JsonConvert.DeserializeObject<VideoDataObject>(Intent?.GetStringExtra("PostLiveStream") ?? ""); break;
				}
					...
					case true:
						{
							if (PostObject != null)
							{
								...
								StreamRateLevel.SetImageURI(Uri.Parse(PostObject.Owner?.OwnerClass?.Avatar));
					...
					default:
						{
							if (LiveStreamViewerObject != null)
							{
								...
								StreamRateLevel.SetImageURI(Uri.Parse(LiveStreamViewerObject.Owner?.OwnerClass?.Avatar));
		...
		private async void MShareBtnOnClick(object sender, EventArgs e)
		{
			try
			{
				...
				await CrossShare.Current.Share(new ShareMessage
				{
					...
					Url = "https://" + GetText(Resource.String.ApplicationShortUrl) + "/" + video.VideoId //video.Url
				});
			}
			catch (Exception exception)
			{
		...
		private async void MSendBtnOnClick(object sender, EventArgs e)
		{
			try
			{
				...
				if (Methods.CheckConnectivity())
				{
					...
					var postId = MAdapter.CommentList.FirstOrDefault(a => a.VideoId == Convert.ToInt32(VideoId))?.VideoId ?? 0;
					...
					var (apiStatus, respond) = await RequestsAsync.Comments.AddCommentAsync(video.Id, text);
					if (apiStatus == 200)
					{
						if (respond is MessageIdObject result)
						{
							var date = MAdapter.CommentList.FirstOrDefault(a => a.Id == comment.Id) ??
									   MAdapter.CommentList.FirstOrDefault(x => x.Id == result.Id);
							if (date != null)
							{
								date.Id = result.Id;

								index = MAdapter.CommentList.IndexOf(
									MAdapter.CommentList.FirstOrDefault(a => a.Id == unixTimestamp));
								if (index > -1)
								{
					...
					default:
						{
							if (LiveStreamViewerObject != null)
							{
								GlideImageLoader.LoadImage(this, LiveStreamViewerObject.Owner?.OwnerClass?.Avatar, MAvatarBg, ImageStyle.CenterCrop, ImagePlaceholders.DrawableUser);
								GlideImageLoader.LoadImage(this, LiveStreamViewerObject.Owner?.OwnerClass?.Avatar, MAvatar, ImageStyle.CircleCrop, ImagePlaceholders.DrawableUser);
							}
		...
		private void InitStreamerInfo()
		{
			try
			{
				...
				if (LiveStreamViewerObject != null)
				{
					GlideImageLoader.LoadImage(this, LiveStreamViewerObject.Owner?.OwnerClass?.Avatar, streamerAvatar, ImageStyle.CircleCrop, ImagePlaceholders.DrawableUser);
...
#pragma warning restore 618
		...
		public void OnError(int err)
		{
			RunOnUiThread(() =>
			{
				try
				{
					...
					var dialog = new MaterialAlertDialogBuilder(this);
					dialog.SetTitle(GetText(Resource.String.Lbl_Error_Code) + " " + err);
					...
					dialog.SetNeutralButton(GetText(Resource.String.Lbl_ContactUs), (materialDialog, action) =>
					{
						try
						{
							new IntentController(this).OpenBrowserFromApp(InitializePlayTube.WebsiteUrl + "/contact-us");
							Finish();
						}
						catch (Exception e)
						{
		...
		//        {
		//            HandlerThread handlerThread = new HandlerThread(VideoId + "_thumbnail");
		//            handlerThread.Start();
		...
		//        {
		//            var pathImage = Methods.MultiMedia.Export_Bitmap_As_Image(SurfaceBitmap, VideoId + "_thumbnail", Methods.Path.FolderDcimImage);
		//            if (!string.IsNullOrEmpty(pathImage))
		//            {
		//                PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Posts.CreateLiveThumbnail(VideoId, pathImage) });
		//            }
		//        }
		//    }
		//    catch (Exception e)
