...
using PlayTube.PlayTubeClient.Classes.Payment;
...
namespace PlayTube.Payment
{
	...
	public class PaymentCardDetailsActivity : ComponentActivity, IPaymentLauncher.IPaymentResultCallback
	{
		...
		private void InitWalletStripe()
		{
			try
			{
				string stripePublishableKey = ListUtils.MySettingsList?.StripeId ?? "";
		...
		private async void OnSuccess()
		{
			try
			{
				if (Methods.CheckConnectivity())
				{
					...
					{
						//wael change to api after new update 
						var (apiStatus, respond) = await RequestsAsync.Payment.TopWalletPaypalAsync(Price);
						//var (apiStatus, respond) = await RequestsAsync.Payment.TopWalletStripeAsync(SessionId);
						switch (apiStatus)
						{
							case 200:
		...
		private async Task StripeCreatePayment()
		{
			try
			{
				...
				{
					var currency = ListUtils.MySettingsList?.StripeCurrency ?? "USD";
					var stripeSecretKey = ListUtils.MySettingsList?.StripeSecret ?? "";
					var priceInt = Convert.ToInt32(Price) * 100;
					var (apiStatus, respond) = await RequestsAsync.Payment.StripeCreatePaymentIntentsAsync(stripeSecretKey, priceInt.ToString(), currency);
					if (apiStatus == 200)
					{
		...
		private async Task CreateStripeHash()
		{
			...
			{
				var (apiStatus, respond) = await RequestsAsync.Payment.CreateStripeAsync(Price);
				if (apiStatus == 200)
				{
