...
namespace PlayTube.Activities.Live.Page
{
	...
	public class RtcBaseActivity : AppCompatActivity
	{
		...
		protected void JoinChannel()
		{
			...
			{
				string token = null!;
				if (!string.IsNullOrEmpty(ListUtils.MySettingsList?.AgoraAppCertificate))
				{
					...
					token = RtcTokenBuilder.BuildTokenWithUid(AppSettings.AppIdAgoraLive, ListUtils.MySettingsList?.AgoraAppCertificate, channelName, uid, RtcTokenBuilder.Role.RolePublisher, timestamp);
				}
