...
namespace PlayTube.Payment
{
	...
	public class AuthorizeNetPaymentActivity : BaseActivity
	{
			...
			try
			{
				if (MultilineWidget.ValidateAllFields() && !string.IsNullOrEmpty(EtName.Text))
				{
					...
					if (Methods.CheckConnectivity())
					{
						...
						{
							var (apiStatus, respond) = await RequestsAsync.Payment.TopWalletAuthorizeNetAsync(cardNumber, ExpMonth, ExpYear, cardCvv, Price);
							switch (apiStatus)
							{
								case 200:
		...
		private bool InitWalletAuthorizeNet()
		{
			try
			{
				var authorizeLoginId = ListUtils.MySettingsList?.AuthorizeLoginId ?? "";
				if (!string.IsNullOrEmpty(authorizeLoginId))
				{
					//API_LOGIN_ID = ListUtils.MySettingsList?.AuthorizeLoginId;
					//CLIENT_KEY = ListUtils.MySettingsList?.AuthorizeTransactionKey;

					//var mode = ListUtils.MySettingsList?.AuthorizeTestMode;
					//if (mode == "SANDBOX") 
					//{
