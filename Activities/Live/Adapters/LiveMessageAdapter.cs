...
namespace PlayTube.Activities.Live.Adapters
{
	public class LiveMessageAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
	{
		...
		public ObservableCollection<CheckLiveCommentsObject.CommentDataObject> CommentList = new ObservableCollection<CheckLiveCommentsObject.CommentDataObject>();
					...
					case LiveMessageAdapterViewHolder holder:
						{
							...
							if (item != null)
							{
								...
								holder.TxtName.Text = AppTools.GetNameFinal(item.UserData);
								holder.TxtMessage.Text = Methods.FunString.DecodeString(item.Text);
							}
		...
		public CheckLiveCommentsObject.CommentDataObject GetItem(int position)
		{
